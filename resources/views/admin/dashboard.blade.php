...
<form method="POST" action="{{ route('admin.setAdmin', $user->id) }}">
    @csrf
    @method('PATCH')
    <button type="submit" class="btn btn-sm btn-primary">Assegna ruolo Admin</button>
</form>
<form method="POST" action="{{ route('admin.setAdmin', $user->id) }}">
    @csrf
    @method('PATCH')
    <button type="submit">Assegna Admin</button>
</form>